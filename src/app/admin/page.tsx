...
import {
  ...
  setDoc, getCountFromServer, limit, startAfter, QueryDocumentSnapshot,
...
const ANN_PAGE = 20; // 공지 탭 한 번에 로드할 수
...
function Toast({ msg }: { msg: string }) {
  return <div style={{ position:"fixed", bottom:32, left:"50%", transform:"translateX(-50%)", background:"rgba(26,20,18,0.9)", color:"#fff", padding:"10px 20px", borderRadius:24, fontSize:13, fontWeight:600, zIndex:9999, whiteSpace:"nowrap", pointerEvents:"none" }}>{msg}</div>;
}
function EmptyBox({ icon, text }: { icon: string; text: string }) {
  return <div style={{ background:"#fff", borderRadius:16, padding:"40px 20px", textAlign:"center", boxShadow:"0 1px 6px rgba(0,0,0,0.05)" }}><div style={{ fontSize:44, marginBottom:12 }}>{icon}</div><p style={{ fontSize:14, color:MUTED }}>{text}</p></div>;
}
function Badge({ text, color }: { text: string; color: string }) {
  return <div style={{ background:color+"1A", borderRadius:20, padding:"2px 8px", display:"inline-block" }}><span style={{ fontSize:10, fontWeight:700, color }}>{text}</span></div>;
}
function Row({ label, value }: { label: string; value: string }) {
  return <div style={{ display:"flex", justifyContent:"space-between", padding:"10px 0", borderBottom:"1px solid #F0EBE3" }}><span style={{ fontSize:13, color:MUTED }}>{label}</span><span style={{ fontSize:13, color:INK, fontWeight:500, maxWidth:"60%", textAlign:"right", wordBreak:"break-all" }}>{value}</span></div>;
}
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return <div style={{ marginBottom:18 }}><p style={{ fontSize:11, fontWeight:700, color:MUTED, letterSpacing:0.5, marginBottom:6, textTransform:"uppercase" }}>{title}</p><div style={{ background:WARM, borderRadius:12, padding:"4px 14px" }}>{children}</div></div>;
}
function StatCard({ label, value, color }: { label: string; value: number; color: string }) {
  return <div style={{ flex:1, background:"#fff", borderRadius:10, padding:12, textAlign:"center", border:`1px solid ${BORDER}` }}><p style={{ fontSize:22, fontWeight:700, color }}>{value}</p><p style={{ fontSize:11, color:MUTED, marginTop:2 }}>{label}</p></div>;
}
...
function UserDetailModal({ user, onClose, onToast }: { user: UserItem; onClose: ()=>void; onToast: (m:string)=>void }) {
  ...
  useEffect(() => {
    adminFetch(`/api/admin/user/${user.id}`).then(r=>r.json()).then(setAuthInfo).catch(console.error);
    if (user.coupleId) {
      getDoc(doc(db,"couples",user.coupleId)).then(async snap=>{
        if (!snap.exists()) return;
        const d=snap.data(); const partnerUid=d.user1Uid===user.id?d.user2Uid:d.user1Uid; if (!partnerUid) return;
        const ps=await getDoc(doc(db,"users",partnerUid));
        if (ps.exists()) setCoupleInfo({partnerName:ps.data().name??"이름 없음",partnerUid,startDate:d.startDate??"—"});
        setCoupleInfoLoaded(true);
      }).catch(()=>setCoupleInfoLoaded(true));
    } else { setCoupleInfoLoaded(true); }
    getDocs(query(collection(db,"community"),where("authorUid","==",user.id))).then(snap=>{
      const list=snap.docs.map(d=>{const v=d.data();return{id:d.id,name:v.name??"",emoji:v.emoji??"🍽️",likes:v.likeCount??0,createdAt:v.createdAt??""};}).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));
      setPosts(list);
      if (!list.length) return;
      const ids=list.map(p=>p.id); const chunks:string[][]=[];
      for (let i=0;i<ids.length;i+=30) chunks.push(ids.slice(i,i+30));
      Promise.all(chunks.map(c=>getCountFromServer(query(collection(db,"community_reports"),where("postId","in",c))))).then(s=>setReportTotal(s.reduce((sum,x)=>sum+x.data().count,0)));
    });
  }, [user.id]);

  const toggleRole = async () => { const r=user.role==="admin"?"user":"admin"; await updateDoc(doc(db,"users",user.id),{role:r}); onToast(`역할을 ${r==="admin"?"관리자":"일반 유저"}로 변경했어요`); onClose(); };
  const changePw   = async () => {
    if (newPw.length<6) { onToast("비밀번호는 6자 이상이어야 해요"); return; }
    setPwLoading(true);
    try { const res=await adminFetch(`/api/admin/user/${user.id}`,{method:"PATCH",body:JSON.stringify({password:newPw})}); if(res.ok){onToast("✅ 비밀번호를 변경했어요");setNewPw("");}else{const e=await res.json();onToast(`❌ ${e.error??"변경 실패"}`);} }
    finally { setPwLoading(false); }
  };
  const fmt=(s:string|null|undefined)=>{if(!s)return"—";try{return new Date(s).toLocaleString("ko-KR",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"});}catch{return s;}};
      ...
      <div style={{ background:"#fff", borderRadius:"20px 20px 0 0", width:"100%", maxWidth:480, maxHeight:"90vh", overflowY:"auto", padding:"20px 20px 40px" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:40, height:40, borderRadius:"50%", background:user.role==="admin"?PURPLE+"20":WARM, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20 }}>{user.role==="admin"?"👑":"👤"}</div>
            <div><p style={{ fontSize:16, fontWeight:700, color:INK }}>{user.name}</p><div style={{ display:"flex", gap:4, marginTop:2 }}><Badge text={user.role==="admin"?"관리자":"유저"} color={user.role==="admin"?PURPLE:SAGE}/>{user.coupleId&&<Badge text="커플 연동" color={ROSE}/>}</div></div>
          </div>
        ...
        <Section title="계정 정보">
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:`1px solid ${BORDER}` }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              <span style={{ fontSize:13, fontWeight:600, color:authInfo?.emailVerified?"#059669":"#EF4444" }}>{authInfo==null?"로딩 중...":authInfo.emailVerified?"✅ 인증됨":"❌ 미인증"}</span>
              {authInfo&&!authInfo.emailVerified&&<button onClick={async()=>{const res=await adminFetch(`/api/admin/user/${user.id}`,{method:"PATCH",body:JSON.stringify({emailVerified:true})});if(res.ok){setAuthInfo(p=>p?{...p,emailVerified:true}:p);onToast("✅ 강제 인증 완료");}else{onToast("❌ 인증 처리 실패");}}} style={{ padding:"4px 10px", background:"#05966920", border:"1px solid #05966960", borderRadius:8, color:"#059669", fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>강제 인증</button>}
            </div>
          </div>
        ...
        <Section title="역할 관리">
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 0" }}>
            ...
            <button onClick={toggleRole} style={{ padding:"7px 14px", background:user.role==="admin"?RED+"1A":PURPLE+"1A", border:`1px solid ${user.role==="admin"?RED:PURPLE}60`, borderRadius:10, color:user.role==="admin"?RED:PURPLE, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>{user.role==="admin"?"관리자 제거":"관리자 등록"}</button>
          </div>
        </Section>
        <Section title="비밀번호 변경">
          <div style={{ display:"flex", gap:8, marginTop:4 }}>
            <input type="password" placeholder="새 비밀번호 (6자 이상)" value={newPw} onChange={e=>setNewPw(e.target.value)} style={inp}/>
            <button onClick={changePw} disabled={pwLoading} style={{ padding:"10px 14px", background:newPw.length>=6?BLUE:"#C0B8B0", border:"none", borderRadius:10, color:"#fff", fontSize:12, fontWeight:600, cursor:newPw.length>=6?"pointer":"default", fontFamily:"inherit", whiteSpace:"nowrap" }}>{pwLoading?"변경 중...":"변경"}</button>
          </div>
        </Section>
        <Section title="커플 정보">
          {!coupleInfoLoaded?<div style={{ padding:"10px 0" }}><span style={{ fontSize:13, color:MUTED }}>로딩 중...</span></div>:coupleInfo?<><Row label="교제 시작일" value={coupleInfo.startDate}/><Row label="파트너 이름" value={coupleInfo.partnerName}/><Row label="파트너 UID" value={coupleInfo.partnerUid.slice(0,16)+"..."}/></>:<div style={{ padding:"10px 0" }}><span style={{ fontSize:13, color:MUTED }}>커플 연동 없음</span></div>}
        </Section>
        <Section title="활동 통계">
          <div style={{ display:"flex", gap:10, marginTop:4 }}><StatCard label="게시 글" value={posts.length} color={SAGE}/><StatCard label="총 신고 횟수" value={reportTotal} color={reportTotal>0?RED:MUTED}/></div>
        </Section>
        {posts.length>0&&<Section title="게시 글 목록">{posts.map(p=><div key={p.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 0", borderBottom:`1px solid ${BORDER}` }}><span style={{ fontSize:20 }}>{p.emoji}</span><div style={{ flex:1, minWidth:0 }}><p style={{ fontSize:13, fontWeight:600, color:INK, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{p.name}</p><p style={{ fontSize:11, color:MUTED, marginTop:2 }}>{p.createdAt?.slice(0,10)} · ❤️ {p.likes}</p></div></div>)}</Section>}
      </div>
    </div>
  );
}
...
export default function AdminPage() {
  ...
  const [annLoadMore, setAnnLoadMore] = useState(false);
  const [lastAnnDoc,  setLastAnnDoc]  = useState<QueryDocumentSnapshot|null>(null);
  ...
  useEffect(() => {
    (async()=>{
      try {
        const [r,p,c]=await Promise.all([getCountFromServer(query(collection(db,"community_reports"))),getCountFromServer(query(collection(db,"community"))),getCountFromServer(query(collection(db,"contacts")))]);
  ...
  useEffect(() => {
    ...
    switch (tab) {
      case "reports": unsub=onSnapshot(query(collection(db,"community_reports"),orderBy("reportedAt","desc")),s=>{const d=s.docs.map(x=>({id:x.id,...x.data()} as ReportItem));setReports(d);setBadgeCounts(p=>({...p,reports:d.filter(r=>r.status==="pending").length}));}); break;
      case "posts":   unsub=onSnapshot(query(collection(db,"community"),orderBy("createdAt","desc")),s=>{const d=s.docs.map(x=>{const v=x.data();return{id:x.id,name:v.name??"",emoji:v.emoji??"🍽️",coupleLabel:v.showAuthorName===false?"익명 커플":(v.authorName?`${v.authorName}의 추천`:"커플 추천"),likes:v.likeCount??0,authorUid:v.authorUid??""} as PostItem;});setPosts(d);setBadgeCounts(p=>({...p,posts:d.length}));}); break;
      case "faq":     unsub=onSnapshot(query(collection(db,"faq"),orderBy("order","asc")),s=>setFaqs(s.docs.map(x=>({id:x.id,...x.data()} as FAQItem)))); break;
      case "contacts":unsub=onSnapshot(query(collection(db,"contacts"),orderBy("createdAt","desc")),s=>{const d=s.docs.map(x=>({id:x.id,...x.data()} as ContactItem));setContacts(d);setBadgeCounts(p=>({...p,contacts:d.filter(c=>c.status==="pending").length}));}); break;
      case "users":   unsub=onSnapshot(collection(db,"users"),s=>setUsers(s.docs.map(x=>{const v=x.data();return{id:x.id,name:v.name??"이름 없음",role:v.role??"user",coupleId:v.coupleId??null,profileImgUrl:v.profileImgUrl??null} as UserItem;}).sort((a,b)=>{if(a.role==="admin"&&b.role!=="admin")return -1;if(a.role!=="admin"&&b.role==="admin")return 1;return a.name.localeCompare(b.name);}))); break;
      case "config":  unsub=onSnapshot(doc(db,"config","app"),s=>{if(s.exists()){const d=s.data();const cfg={appVersion:d.appVersion??"1.0.0",supportEmail:d.supportEmail??"",notice:d.notice??"",companyName:d.companyName??"",termsDate:d.termsDate??""};setConfig(cfg);setCfgDraft(cfg);}}); break;
      case "announce":
        /* ★ 공지 탭 — 첫 ANN_PAGE개만 실시간 구독 */
        unsub=onSnapshot(
          query(collection(db,"announcements"),orderBy("pinned","desc"),orderBy("createdAt","desc"),limit(ANN_PAGE)),
          s=>{
            setAnnounces(s.docs.map(x=>{const v=x.data();return{id:x.id,title:v.title??"",body:v.body??"",type:v.type??"notice",pinned:v.pinned??false,visible:v.visible??true,startAt:v.startAt??"",endAt:v.endAt??"",imgUrls:v.imgUrls??[],createdAt:v.createdAt??""} as AnnounceItem;}));
            setAnnHasMore(s.docs.length===ANN_PAGE);
            setLastAnnDoc(s.docs[s.docs.length-1]??null);
          }
        );
        break;
    }
  ...
  const loadMoreAnn = useCallback(async () => {
    if (!lastAnnDoc||annLoadMore) return;
    setAnnLoadMore(true);
    try {
      const snap = await getDocs(query(collection(db,"announcements"),orderBy("pinned","desc"),orderBy("createdAt","desc"),startAfter(lastAnnDoc),limit(ANN_PAGE)));
      ...
      setAnnounces(prev=>[...prev,...more]);
      setAnnHasMore(snap.docs.length===ANN_PAGE);
      setLastAnnDoc(snap.docs[snap.docs.length-1]??null);
    } finally { setAnnLoadMore(false); }
  }, [lastAnnDoc, annLoadMore]);

  const deletePost    = useCallback(async (id:string,name:string)=>{await deleteDoc(doc(db,"community",id));showToast(`"${name}" 삭제됨`);}, []);
  const resolveReport = useCallback(async (id:string)=>{await updateDoc(doc(db,"community_reports",id),{status:"resolved"});showToast("신고 처리 완료");}, []);
  ...
  const deleteFaq = useCallback(async (id:string)=>{await deleteDoc(doc(db,"faq",id));showToast("FAQ 삭제됨");}, []);
  const doneContact = useCallback(async (id:string)=>{await updateDoc(doc(db,"contacts",id),{status:"done"});showToast("문의 처리 완료");}, []);
  const saveConfig  = useCallback(async ()=>{await setDoc(doc(db,"config","app"),cfgDraft,{merge:true});setCfgEdit(false);showToast("설정 저장됨");}, [cfgDraft]);

  const sendPushAll = async () => {
    if (!pushTitle.trim()||!pushBody.trim()) return;
    setPushLoading(true);
    try{const res=await adminFetch("/api/notify-all",{method:"POST",body:JSON.stringify({title:pushTitle,body:pushBody})});const j=await res.json();if(res.ok){showToast(`✅ ${j.sent}명에게 발송`);setPushTitle("");setPushBody("");}else{showToast(`❌ ${j.error??"발송 실패"}`);}}
  ...
  const saveAnnounce = useCallback(async ()=>{
    ...
    try {
      ...
      let newUrls:string[]=[];
      if (annFiles.length) { const tempId=annEdit?.id??`tmp_${Date.now()}`; newUrls=await uploadAnnounceImages(annFiles,tempId); }
      const finalImgUrls=[...annDraft.imgUrls,...newUrls];
      if (annEdit){await updateDoc(doc(db,"announcements",annEdit.id),{...annDraft,imgUrls:finalImgUrls});showToast("공지 수정 완료");}
      else{await addDoc(collection(db,"announcements"),{...annDraft,imgUrls:finalImgUrls,createdAt:new Date().toISOString()});showToast("공지 등록됨");}
      resetAnnForm();
    } catch{showToast("❌ 저장 실패");}
    finally{setAnnUploading(false);}
  }, [annDraft,annEdit,annFiles,annDelUrls]);

  const deleteAnnounce=useCallback(async (a:AnnounceItem)=>{if(a.imgUrls?.length)await deleteAnnounceImages(a.imgUrls);await deleteDoc(doc(db,"announcements",a.id));showToast("공지 삭제됨");}, []);
  const toggleVisible =useCallback(async (a:AnnounceItem)=>{await updateDoc(doc(db,"announcements",a.id),{visible:!a.visible});showToast(a.visible?"비공개로 전환":"공개로 전환");}, []);
    ...
    <div style={{ minHeight:"100vh", background:"#F5F0EB", maxWidth:480, margin:"0 auto", fontFamily:"inherit", paddingBottom:40 }}>
      ...
      <div style={{ background:INK, padding:"14px 20px 0", position:"sticky", top:0, zIndex:20 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
          ...
          <div style={{ background:PURPLE, borderRadius:20, padding:"3px 10px" }}><span style={{ fontSize:10, fontWeight:700, color:"#fff" }}>{myName} · ADMIN</span></div>
        </div>
      ...
      <div style={{ padding:"16px 16px 0" }}>
        ...
        {tab==="reports"&&(reports.length===0?<EmptyBox icon="🎉" text="처리 대기 중인 신고가 없어요"/>:reports.map(r=>(
          <div key={r.id} style={{ background:"#fff", borderRadius:14, marginBottom:10, padding:"14px 16px", boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", gap:8 }}>
              <div style={{ flex:1 }}><div style={{ display:"flex", gap:6, marginBottom:4 }}><Badge text={r.status==="pending"?"대기":"완료"} color={r.status==="pending"?RED:SAGE}/></div><p style={{ fontSize:14, fontWeight:600, color:INK }}>{r.postName}</p><p style={{ fontSize:12, color:MUTED, marginTop:2 }}>사유: {r.reason}</p><p style={{ fontSize:11, color:"#C0B8B0", marginTop:2 }}>{r.reportedAt?.slice(0,10)}</p></div>
              {r.status==="pending"&&<div style={{ display:"flex", flexDirection:"column", gap:6 }}><button onClick={()=>resolveReport(r.id)} style={{...btnStyle(SAGE)}}>완료</button><button onClick={async()=>{await deleteDoc(doc(db,"community",r.postId));await resolveReport(r.id);showToast("게시물 삭제됨");}} style={{...btnStyle(RED,true)}}>삭제</button></div>}
            </div>
          </div>
        )))}

        {tab==="posts"&&(posts.length===0?<EmptyBox icon="📋" text="게시물이 없어요"/>:posts.map(p=>(
          <div key={p.id} style={{ background:"#fff", borderRadius:14, marginBottom:10, padding:"14px 16px", display:"flex", alignItems:"center", gap:12, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
            ...
            <div style={{ flex:1, minWidth:0 }}><p style={{ fontSize:14, fontWeight:600, color:INK, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{p.name}</p><p style={{ fontSize:11, color:MUTED }}>{p.coupleLabel} · ❤️ {p.likes}</p></div>
          ...
          <>
            <div style={{ background:"#fff", borderRadius:14, padding:"14px 16px", marginBottom:12, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginBottom:12 }}>
                {FAQ_CATEGORIES.map(cat=><button key={cat} onClick={()=>setFaqCat(cat)} style={{ padding:"5px 12px", borderRadius:20, border:`1.5px solid ${faqCat===cat?SAGE:BORDER}`, background:faqCat===cat?SAGE+"20":WARM, color:faqCat===cat?SAGE:MUTED, fontSize:12, fontWeight:faqCat===cat?700:400, cursor:"pointer", fontFamily:"inherit" }}>{cat}</button>)}
              </div>
              ...
              <div style={{ display:"flex", gap:8 }}>
                ...
                <button onClick={saveFaq} style={{ flex:2, padding:"10px 0", background:faqQ&&faqA?SAGE:"#C0B8B0", border:"none", borderRadius:10, color:"#fff", fontSize:13, fontWeight:600, cursor:faqQ&&faqA?"pointer":"default", fontFamily:"inherit" }}>{faqEdit?"수정 완료":"추가"}</button>
              </div>
            </div>
            <div style={{ display:"flex", gap:6, overflowX:"auto", marginBottom:10, paddingBottom:2 }}>
              {FAQ_CATEGORIES.map(cat=>{const count=faqs.filter(f=>f.category===cat).length;const active=faqFilterCat===cat;return<button key={cat} onClick={()=>setFaqFilterCat(cat)} style={{ flexShrink:0, padding:"6px 12px", borderRadius:20, border:`1.5px solid ${active?ROSE:BORDER}`, background:active?ROSE+"15":"#fff", color:active?ROSE:MUTED, fontSize:12, fontWeight:active?700:400, cursor:"pointer", fontFamily:"inherit" }}>{cat}{count>0?` ${count}`:""}</button>;})}
            </div>
            {filteredFaqs.length===0?<EmptyBox icon="❓" text="등록된 FAQ가 없어요"/>:filteredFaqs.map(f=>(
              <div key={f.id} style={{ background:"#fff", borderRadius:14, marginBottom:10, padding:"14px 16px", boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                ...
                <div style={{ display:"flex", gap:8 }}><button onClick={()=>{setFaqEdit(f);setFaqQ(f.question);setFaqA(f.answer);setFaqCat(f.category||FAQ_CATEGORIES[0]);window.scrollTo({top:0,behavior:"smooth"});}} style={{...btnStyle(SAGE)}}>수정</button><button onClick={()=>deleteFaq(f.id)} style={{...btnStyle(RED,true)}}>삭제</button></div>
              </div>
            ))}
          </>
        )}

        {tab==="contacts"&&(contacts.length===0?<EmptyBox icon="📩" text="접수된 문의가 없어요"/>:contacts.map(c=>(
          <div key={c.id} style={{ background:"#fff", borderRadius:14, marginBottom:10, padding:"14px 16px", boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
              <div style={{ flex:1 }}><div style={{ display:"flex", gap:6, marginBottom:4 }}><Badge text={c.status==="pending"?"미처리":"처리완료"} color={c.status==="pending"?ROSE:SAGE}/></div><p style={{ fontSize:14, fontWeight:600, color:INK }}>{c.name}</p><p style={{ fontSize:12, color:MUTED, marginTop:2 }}>{c.email}</p><p style={{ fontSize:13, color:INK, marginTop:8, lineHeight:1.6, whiteSpace:"pre-wrap" }}>{c.message}</p><p style={{ fontSize:11, color:"#C0B8B0", marginTop:6 }}>{c.createdAt?.slice(0,10)}</p></div>
        ...
        {tab==="users"&&(
          <>
            ...
            {filteredUsers.length===0?<EmptyBox icon="👥" text="유저가 없어요"/>:filteredUsers.map(u=>(
              <button key={u.id} onClick={()=>setSelectedUser(u)} style={{ width:"100%", background:"#fff", borderRadius:14, marginBottom:10, padding:"14px 16px", display:"flex", alignItems:"center", gap:12, boxShadow:"0 1px 4px rgba(0,0,0,0.05)", border:"none", cursor:"pointer", fontFamily:"inherit", textAlign:"left" }}>
                <div style={{ width:38, height:38, borderRadius:"50%", background:u.role==="admin"?PURPLE+"20":WARM, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0 }}>{u.role==="admin"?"👑":"👤"}</div>
                <div style={{ flex:1, minWidth:0 }}><p style={{ fontSize:14, fontWeight:600, color:INK }}>{u.name}</p><div style={{ display:"flex", gap:4, marginTop:3 }}><Badge text={u.role==="admin"?"관리자":"유저"} color={u.role==="admin"?PURPLE:MUTED}/>{u.coupleId&&<Badge text="커플" color={ROSE}/>}</div></div>
        ...
        {tab==="announce"&&(
          <>
            {annForm?(
              <div style={{ background:"#fff", borderRadius:14, padding:"16px", marginBottom:12, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                ...
                {annDraft.type==="event"&&(
                  <div style={{ display:"flex", gap:8, marginBottom:10 }}>
                    <div style={{ flex:1 }}><p style={{ fontSize:11, color:MUTED, marginBottom:4 }}>시작일</p><input type="date" value={annDraft.startAt} onChange={e=>setAnnDraft(p=>({...p,startAt:e.target.value}))} style={{...inp}}/></div>
                    <div style={{ flex:1 }}><p style={{ fontSize:11, color:MUTED, marginBottom:4 }}>종료일</p><input type="date" value={annDraft.endAt} onChange={e=>setAnnDraft(p=>({...p,endAt:e.target.value}))} style={{...inp}}/></div>
                  </div>
                )}
                ...
                <div style={{ display:"flex", gap:16, marginBottom:14 }}>
                  <label style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:INK, cursor:"pointer" }}><input type="checkbox" checked={annDraft.pinned} onChange={e=>setAnnDraft(p=>({...p,pinned:e.target.checked}))}/>상단 고정</label>
                  <label style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:INK, cursor:"pointer" }}><input type="checkbox" checked={annDraft.visible} onChange={e=>setAnnDraft(p=>({...p,visible:e.target.checked}))}/>즉시 공개</label>
                </div>
                <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={()=>setAnnForm(true)} style={{ width:"100%", padding:"12px 0", background:ROSE, border:"none", borderRadius:12, color:"#fff", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit", marginBottom:12 }}>+ 새 공지 등록</button>
            )}

            {/* 공지 목록 */}
            {announces.length===0?<EmptyBox icon="📢" text="등록된 공지가 없어요"/>:announces.map(a=>(
            ...
            {/* ★ 더 보기 버튼 */}
            {annHasMore&&(
              <button onClick={loadMoreAnn} disabled={annLoadMore} style={{ width:"100%", padding:"14px 0", background:"#fff", border:`1px solid ${BORDER}`, borderRadius:14, cursor:annLoadMore?"default":"pointer", fontSize:13, fontWeight:600, color:annLoadMore?MUTED:ROSE, fontFamily:"inherit", marginBottom:10, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                {annLoadMore?"불러오는 중...":"공지 더 보기"}
              </button>
            )}
          </>
        )}
        ...
        {tab==="config"&&(
          <>
            <div style={{ background:"#fff", borderRadius:14, padding:"14px 16px", marginBottom:14, boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
              ...
              <button onClick={sendPushAll} disabled={pushLoading||!pushTitle.trim()||!pushBody.trim()} style={{ width:"100%", padding:"12px 0", background:pushTitle&&pushBody?ROSE:"#C0B8B0", border:"none", borderRadius:10, color:"#fff", fontSize:14, fontWeight:700, cursor:pushTitle&&pushBody?"pointer":"default", fontFamily:"inherit" }}>{pushLoading?"발송 중...":"전체 발송"}</button>
            </div>
              ...
              {cfgEdit?(
                <>
                  {(["appVersion","supportEmail","companyName","termsDate"] as const).map(k=>{
                    ...
                    return<div key={k}><label style={{ fontSize:12, color:MUTED, display:"block", marginBottom:4 }}>{labels[k]}</label><input value={cfgDraft[k]} onChange={e=>setCfgDraft(p=>({...p,[k]:e.target.value}))} style={{...inp,marginBottom:10}}/></div>;
                  })}
