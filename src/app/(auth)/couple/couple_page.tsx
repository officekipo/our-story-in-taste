    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 320, background: "#fff", borderRadius: 24, padding: "28px 24px", textAlign: "center", animation: "scaleIn 0.2s ease" }}>
        ...
        <p style={{ fontSize: 13, color: MUTED, lineHeight: 1.7, marginBottom: 6 }}>
          {partnerName
            ? <><strong style={{ color: ROSE }}>{partnerName}</strong>님과의 연동이 해제됩니다.</>
            : "파트너와의 연동이 해제됩니다."}
        </p>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onClose} disabled={loading}
            style={{ flex: 1, padding: 13, background: WARM, border: `1px solid ${BORDER}`, borderRadius: 12, color: MUTED, fontSize: 14, cursor: "pointer", fontFamily: "inherit" }}>
            취소
          </button>
          <button onClick={onConfirm} disabled={loading}
            style={{ flex: 2, padding: 13, background: loading ? "#C0B8B0" : RED, border: "none", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 700, cursor: loading ? "default" : "pointer", fontFamily: "inherit" }}>
            {loading ? "해제 중…" : "연동 해제"}
          </button>
        </div>
      </div>
...
export default function CouplePage() {
  ...
  const { myUid, coupleId, partnerName, setCoupleId, setStartDate: setAuthStartDate, setAuth } = useAuthStore();
  ...
  const handleCreate = async () => {
    ...
    setStatus("loading");
    try {
      const { coupleId: id, inviteCode: code } = await createCouple(myUid, startDate);
      setCoupleId(id);
  ...
  // ★ 이미 연동된 상태 — 연동 현황 + 해제 버튼
  if (coupleId) {
    return (
      <div>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: INK, marginBottom: 6 }}>커플 연동</h2>
        <p style={{ fontSize: 13, color: MUTED, marginBottom: 24 }}>현재 파트너와 연동 중이에요 💑</p>

        <div style={{ background: "#fff", borderRadius: 20, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.07)", marginBottom: 16, textAlign: "center" }}>
          <div style={{ fontSize: 48, marginBottom: 12 }}>💑</div>
          <p style={{ fontSize: 16, fontWeight: 700, color: INK, marginBottom: 6 }}>
            {partnerName ? `${partnerName}님과 연동 중` : "파트너와 연동 중"}
          </p>
          <p style={{ fontSize: 13, color: MUTED }}>함께 맛집을 기록하고 있어요 🍽️</p>
        </div>
