    ...
    >
      <div
        style={{
          ...
          height: 36,
          borderRadius: 10,
          background: danger ? "#FFF5F5" : CREAM,
          display: "flex",
          alignItems: "center",
      ...
      {rightEl ??
        (onClick && (
          <span style={{ fontSize: 18, color: "#C0B8B0", flexShrink: 0 }}>
            ›
          </span>
        ))}
    </button>
...
function EditNamePopup({
  ...
  return (
    <Popup onClose={onClose}>
      <p
        style={{ fontSize: 16, fontWeight: 700, color: INK, marginBottom: 16 }}
      >
        닉네임 변경
      </p>
      <input
...
function InvitePopup({ onClose }: { onClose: () => void }) {
  ...
  const myCode = "TASTE-SAMPLE"; // TODO: Firestore에서 실제 코드 로드
  ...
  return (
    <Popup onClose={onClose}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        ...
        {sub && (
          <p style={{ fontSize: 12, color: "#EF4444", marginTop: 6 }}>{sub}</p>
        )}
      </div>
      <div style={{ display: "flex", gap: 10 }}>
        <button
...
export default function SettingsPage() {
  ...
  const { myName, partnerName, startDate, role, setMyName, setStartDate } =
    useAuthStore();
  const dday = calcDDay(startDate);

  /* 팝업 상태 */
  const [modal, setModal] = useState<
    "name" | "date" | "invite" | "logout" | "withdraw" | null
  >(null);
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              <p style={{ fontSize: 19, fontWeight: 800, color: INK }}>
                {myName}
              </p>
              {role === "admin" && (
                <div
      ...
      {notifPermission === "denied" && (
        ...
        >
          <p style={{ fontSize: 12, color: "#EF4444" }}>
            ⚠️ 알림 권한이 거부됐습니다. 기기 설정에서 알림을 허용해주세요.
          </p>
        </div>
      )}
      ...
      <div style={{ borderTop: `1px solid ${BORDER}` }}>
        <Row
          ...
          rightEl={
            <Toggle
              on={notif.partnerRecord}
              onToggle={() => toggleNotif("partnerRecord")}
            />
          }
        />
        <Row
          ...
          rightEl={
            <Toggle
              on={notif.wishlistAdd}
              onToggle={() => toggleNotif("wishlistAdd")}
            />
          }
        />
        <Row
          ...
          rightEl={
            <Toggle
              on={notif.anniversary}
              onToggle={() => toggleNotif("anniversary")}
            />
          }
        />
      </div>
      ...
      <div style={{ borderTop: `1px solid ${BORDER}` }}>
        ...
        <Row
          icon="📍"
          label="위치기반 서비스 이용약관"
          onClick={() => router.push("/settings/location-terms")}
        />
        <Row
          icon="💬"
          label="고객센터 / 문의하기"
            ...
            <Row
              ...
              rightEl={
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  >
                    <span
                      style={{ fontSize: 10, fontWeight: 700, color: ROSE }}
                    >
                      ADMIN
                    </span>
                  </div>
