  ...
  return (
    <Popup onClose={onClose}>
      ...
      <input value={val} onChange={(e)=>{ setVal(e.target.value); setErr(""); }} maxLength={10} autoFocus
        style={{ width:"100%", padding:"12px 14px", background:WARM, border:`1.5px solid ${err?"#EF4444":BORDER}`, borderRadius:10, fontSize:14, color:INK, fontFamily:"inherit", outline:"none", boxSizing:"border-box", marginBottom:4 }} />
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:err?6:16 }}>
        {err ? <p style={{ fontSize:11, color:"#EF4444" }}>{err}</p> : <span />}
        <p style={{ fontSize:11, color:MUTED }}>{val.length}/10</p>
      </div>
      {err && <div style={{ marginBottom:12 }} />}
...
function InvitePopup({ onClose }: { onClose:()=>void }) {
  ...
  const handleJoin = async () => {
    ...
    if (myCode !== "불러오는 중..." && myCode !== "미연동" && trimmed === myCode) {
      setJoinStatus("self");
      setJoinMsg("😅 본인이 만든 코드예요. 파트너의 코드를 입력해주세요.");
      return;
    }
    ...
    try {
      ...
      const msg = e.message ?? "";
      if (msg.includes("not found")||msg.includes("없")||msg.includes("invalid")) {
        setJoinStatus("notfound"); setJoinMsg("❌ 없는 코드예요. 다시 확인해주세요.");
      } else {
        setJoinStatus("error"); setJoinMsg("❌ 연동 실패. 다시 시도해주세요.");
      }
    }
  };
  ...
  const ss = statusStyle[joinStatus];
          ...
          <div>
            <input placeholder="TASTE-XXXXXX" value={code}
              ...
              style={{ width:"100%", padding:"12px 14px", background:WARM,
                border:`1.5px solid ${joinStatus==="notfound"||joinStatus==="error"?"#EF4444":joinStatus==="success"?SAGE:joinStatus==="self"?"#F59E0B":BORDER}`,
  ...
  return (
    <Popup onClose={onClose}>
      <div style={{ textAlign:"center", marginBottom:20 }}>
        ...
        {sub && <p style={{ fontSize:12, color:"#EF4444", marginTop:4 }}>{sub}</p>}
      </div>
      <div style={{ display:"flex", gap:10 }}>
        <button onClick={onClose} style={{ flex:1, padding:12, background:WARM, border:`1px solid ${BORDER}`, borderRadius:12, color:MUTED, fontSize:14, cursor:"pointer", fontFamily:"inherit" }}>취소</button>
        <button onClick={onConfirm} style={{ flex:2, padding:12, background:danger?"#EF4444":ROSE, border:"none", borderRadius:12, color:"#fff", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>{confirmLabel}</button>
      </div>
    </Popup>
  );
}
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (err?.code === "storage/unauthorized") {
        setErrMsg("Storage 규칙을 업데이트해주세요. (FIREBASE_STORAGE_RULES.txt 참고)");
      } else {
        setErrMsg("업로드 실패. 다시 시도해주세요.");
      }
    ...
    <>
      ...
      {errMsg && (
        <div style={{ margin:"0 16px 8px", padding:"10px 14px", background:"#FFF0F0", borderRadius:10, border:"1px solid rgba(239,68,68,0.2)" }}>
          <p style={{ fontSize:12, color:"#EF4444" }}>⚠️ {errMsg}</p>
        </div>
      )}
    </>
  );
    ...
    <div style={{ minHeight:"100vh", background:"#F5F0EB", maxWidth:480, margin:"0 auto", fontFamily:"inherit", paddingBottom:40 }}>
      ...
      {notifPermission==="denied" && (
        <div style={{ margin:"0 16px 8px", background:"#FFF0F0", border:"1px solid rgba(239,68,68,0.2)", borderRadius:12, padding:"10px 14px" }}>
          <p style={{ fontSize:12, color:"#EF4444" }}>⚠️ 알림 권한이 거부됐습니다. 기기 설정에서 허용해주세요.</p>
        </div>
      )}
      <div style={{ borderTop:`1px solid ${BORDER}` }}>
      ...
      {modal==="logout"  && <ConfirmPopup emoji="👋" title="로그아웃 하시겠어요?" desc="다시 로그인하면 기록이 그대로 있어요." confirmLabel="로그아웃" onConfirm={()=>{ close(); router.push("/login"); }} onClose={close} />}
