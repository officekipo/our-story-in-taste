...
//
//  수정사항:
//    ★ 헤더 전체 높이 축소 (BottomNav 공간 확보)
//    ★ scrolled prop으로 필터 바 숨김 (AppShell에서 main 스크롤 감지)
//    ★ 검색바: showSearch 토글로만 제어
//    ★ myName / partnerName 8자 초과 시 말줄임 처리
// ============================================================
"use client";
...
export function Header({
  ...
  // 필터 바는 스크롤 내렸을 때 숨김
  const showFilterBar = isVisited && !scrolled;
    ...
    <header style={{ background: "#fff", borderBottom: `1px solid ${BORDER}`, padding: "0 16px", position: "sticky", top: 0, zIndex: 20, flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 8, paddingBottom: 7 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <button onClick={() => router.push("/settings")} aria-label="설정" className="tap" style={{ width: 32, height: 32, borderRadius: "50%", background: ROSE, border: "2.5px solid #F2D5CC", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", padding: 0, boxShadow: "0 2px 8px rgba(201,107,82,0.3)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingBottom: 9 }}>
        ...
        {isVisited && viewMode && onViewMode && (
          <div style={{ display: "flex", background: "#F5F0EB", borderRadius: 10, padding: "3px 3px 2px", border: `1px solid ${BORDER}`, gap: 2, flexShrink: 0 }}>
      ...
      {showFilterBar && (
        <div style={{ borderTop: `1px solid ${BORDER}` }}>
          <div style={{ display: "flex", gap: 7, padding: "8px 3px", overflowX: "auto", alignItems: "center", scrollbarWidth: "none" }}>
            ...
            <button onClick={onToggleSearch} className="tap" style={{ ...(showSearch ? { ...chipActive, background: "#F2D5CC", color: ROSE, outline: `1px solid ${ROSE}` } : chipInactive), marginLeft: "auto" }}>
            ...
            <div style={{ paddingBottom: 8, position: "relative" }}>
              <div style={{ position: "relative" }}>
                <input
                  ...
                  autoFocus
                  style={{ width: "100%", padding: "9px 44px 9px 14px", background: "#FAFAFA", border: `1.5px solid ${BORDER}`, borderRadius: 10, color: INK, fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}
                />
                ...
                <>
                  ...
                  <div style={{ position: "absolute", top: "calc(100% + 4px)", right: 0, zIndex: 99 }}>
                    <DateRangePicker
                      ...
                      onChange={(from, to) => { onFilterDateRange?.(from, to); setShowCalendar(false); }}
