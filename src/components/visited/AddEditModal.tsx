...
// Storage 업로드 함수 (Firebase 연동 후 사용)
// DUMMY_MODE = true 일 때는 FileReader로 로컬 미리보기만 사용
const DUMMY_MODE = false; // page.tsx 와 동일하게 맞춰주세요
...
export function AddEditModal({ onSave }: AddEditModalProps) {
  ...
  const [revisit, setRevisit] = useState<boolean | null>(
    editTarget?.revisit ?? null,
  );
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (DUMMY_MODE) {
      ...
      // Firebase 모드: Storage 실제 업로드
      if (!coupleId) return;
      setUploading(true);
      setUploadPct(0);
      try {
        const { uploadImages } = await import("@/lib/firebase/storage");
        const urls = await uploadImages(coupleId, files, (pct) =>
          setUploadPct(pct),
        );
    ...
    >
      ...
      >
        <p style={{ fontSize: 16, fontWeight: 700, color: INK }}>
          {editTarget?.id ? "맛집 기록 수정" : "새로운 맛집 기록하기"}
        </p>
        <button
          onClick={closeModal}
          style={{
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {imgUrls.map((url, i) => (
              ...
              >
                ...
                />
                <button
                  onClick={() => setImgUrls((p) => p.filter((_, j) => j !== i))}
                  style={{
                    position: "absolute",
                    top: -4,
                    right: -4,
            ...
            {imgUrls.length < 5 && (
              <label
                style={{
                  width: imgUrls.length === 0 ? 120 : 80,
                  height: imgUrls.length === 0 ? 120 : 80,
              ...
              >
                ...
                {imgUrls.length === 0 && (
                  <span style={{ fontSize: 10, color: MUTED }}>사진 추가</span>
                )}
                <input
                  type="file"
                  accept="image/*"
        ...
        <div>
          ...
          <div style={{ position: "relative" }}>
            <select
              ...
              style={{ ...inp, paddingRight: 26, cursor: "pointer" }}
            >
        ...
        <div style={{ position: "relative" }}>
          ...
          <div
            ...
            style={{ ...inp, cursor: "pointer", color: date ? INK : MUTED }}
          >
            {date || "날짜를 선택하세요"}
          </div>
          {showCal && (
              ...
              return (
                <button
                  ...
                  style={{
                    padding: "5px 12px",
                    borderRadius: 20,
                    border: `1.5px solid ${on ? ROSE : BORDER}`,
                    background: on ? "#F2D5CC" : WARM,
                    color: on ? ROSE : MUTED,
                    fontSize: 12,
        ...
        <div>
          ...
          <button
            ...
            style={{
              ...
              marginBottom: 8,
              borderRadius: 12,
              border: `1.5px solid ${revisit === true ? ROSE : BORDER}`,
              background: revisit === true ? "#FDE8E5" : WARM,
              color: revisit === true ? ROSE : MUTED,
              fontSize: 14,
              fontWeight: revisit === true ? 700 : 400,
              cursor: "pointer",
              fontFamily: "inherit",
            }}
          >
          ...
          <button
            ...
            style={{
              ...
              padding: 13,
              borderRadius: 12,
              border: `1.5px solid ${revisit === false ? "#C0B8B0" : BORDER}`,
              background: revisit === false ? "#F0EBE3" : WARM,
              color: MUTED,
              fontSize: 14,
              cursor: "pointer",
              fontFamily: "inherit",
          ...
          >
            <div
              style={{
                ...
                height: 20,
                borderRadius: 5,
                border: `2px solid ${share ? ROSE : "#C0B8B0"}`,
                background: share ? ROSE : "#fff",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                flexShrink: 0,
        ...
        <button
          ...
          style={{
            width: "100%",
            padding: 15,
            background: name && cuisine ? ROSE : "#C0B8B0",
            border: "none",
            borderRadius: 12,
            color: "#fff",
            fontSize: 15,
            fontWeight: 700,
            cursor: name && cuisine ? "pointer" : "default",
            fontFamily: "inherit",
          }}
        >
          {editTarget?.id ? "수정 완료" : "기록 저장하기"}
        </button>
      </div>
    </Modal>
  );
