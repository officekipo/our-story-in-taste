...
//      배경 클릭 or ✕ 버튼으로 닫기
//    - [Fix #1] 라이트박스 힌트 아이콘: 14px → 22px, 컨테이너 44×44px 로 확대
//    - [Fix #2] 라이트박스를 createPortal로 document.body에 직접 렌더링
//      → Modal(zIndex:750) 내부에서 사용 시 stacking context에 갇히는 문제 해결
//      → mounted 가드로 SSR hydration 오류 방지
// ============================================================
"use client";
...
export function ImageSlider({ images, thumbnails, emoji = "🍽️", height = 220, rounded = true, lightbox = false }: ImageSliderProps) {
  ...
  // ★ Fix #2: SSR/CSR 불일치 방지 — 클라이언트 마운트 후에만 Portal 렌더링
  ...
  // ★ Fix #2: 라이트박스를 Portal로 분리 — Modal의 stacking context 탈출
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          color: "#fff", fontSize: 20, cursor: "pointer", zIndex: 1,
          display: "flex", alignItems: "center", justifyContent: "center",
        }}
      >✕</button>
          ...
          {images.map((_, i) => (
            <button
              ...
              style={{
                ...
                background: i === lbIdx ? "#fff" : "rgba(255,255,255,0.35)",
    ...
    <>
      ...
      <div
        style={{
          ...
          backgroundColor: "#F0EBE3", borderRadius, height,
          cursor: lightbox && images.length > 0 ? "zoom-in" : "default",
        }}
          ...
          <>
            ...
            {images.length > 1 && idx > 0 && (
              <button
                ...
                style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", width: 32, height: 32, borderRadius: "50%", background: "rgba(0,0,0,0.4)", border: "none", color: "#fff", fontSize: 18, cursor: "pointer", zIndex: 2, display: "flex", alignItems: "center", justifyContent: "center" }}
              >‹</button>
            )}
            {images.length > 1 && idx < images.length - 1 && (
              <button
                onClick={e => { e.stopPropagation(); go(idx + 1); }}
                style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", width: 32, height: 32, borderRadius: "50%", background: "rgba(0,0,0,0.4)", border: "none", color: "#fff", fontSize: 18, cursor: "pointer", zIndex: 2, display: "flex", alignItems: "center", justifyContent: "center" }}
              >›</button>
            )}
            {images.length > 1 && (
              <div style={{ position: "absolute", bottom: 10, left: "50%", transform: "translateX(-50%)", display: "flex", gap: 6, zIndex: 2 }}>
                {images.map((_, i) => (
                  <button
                    ...
                    style={{ width: i === idx ? 20 : 6, height: 6, borderRadius: 3, background: i === idx ? "#fff" : "rgba(255,255,255,0.5)", border: "none", cursor: "pointer", padding: 0, transition: "all 0.25s cubic-bezier(0.34,1.56,0.64,1)" }}
                  />
                ))}
              </div>
            )}
            {/* ★ 라이트박스 힌트 아이콘 — Fix #1: 44×44 시각 타겟, 아이콘 14→22px */}
            {lightbox && (
              <div style={{ position: "absolute", top: 8, right: 8, width: 44, height: 44, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.40)", borderRadius: 10, zIndex: 2, pointerEvents: "none" }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                  <path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
            )}
          </>
      ...
      {/* ★ Fix #2: Portal로 document.body에 직접 마운트 → Modal stacking context 탈출 */}
      {lightboxPortal}
    </>
  );
}
